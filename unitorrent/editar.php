	...
	<div class="container mt-5">
		<div class="card">
			...
			<div class="card-body">
				...
				<form method="post" action="validation/valida_editar.php">
					<input type="hidden" name="id" value="<?php if(isset($row_usuario['id'])){ echo $row_usuario['id']; } ?>">
					<div class="form-group">
						<label for="exampleInputEmail1">Nome</label>
						<input type="text" class="form-control" name="nome" placeholder="Inserir o nome completo" value="<?php if(isset($row_usuario['nome'])){ echo $row_usuario['nome']; } ?>">
					</div>
					<div class="form-group">
						<label for="exampleInputEmail1">Apelido</label>
						<input type="text" class="form-control" name="apelido" placeholder="Inserir o apelido" value="<?php if(isset($row_usuario['apelido'])){ echo $row_usuario['apelido']; } ?>">
					</div>
					<div class="form-group">
						<label for="exampleInputEmail1">Telefone</label>
						<input type="number" class="form-control" name="telefone" placeholder="Inserir o telefone" value="<?php if(isset($row_usuario['telefone'])){ echo $row_usuario['telefone']; } ?>">
					</div>
					<div class="form-group">
						<label for="exampleInputEmail1">Email</label>
						<input type="email" class="form-control" name="email" placeholder="Inserir o email" value="<?php if(isset($row_usuario['email'])){ echo $row_usuario['email']; } ?>">
					</div>
					<a href="painel.php" class="btn btn-danger">Voltar</a>
					<input name="SendEdit" class="btn btn-primary" type="submit" value="Editar">
				</form>
			</div>
		</div>
	</div>
